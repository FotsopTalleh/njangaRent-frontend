// Student Notifications — Supabase-backed
...
const TYPE_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  appointment_pending:   { label: "New visit",         color: "bg-amber-100 text-amber-700"    },
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <Button variant="ghost" size="sm" className="rounded-xl gap-2" onClick={() => markAllRead.mutate()} disabled={markAllRead.isPending}>
      ...
      {!isLoading && notifications.length === 0 && !error && (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-xs mt-1">You'll be notified about viewings and listing updates.</p>
        </div>
      )}
          ...
          return (
            <div
              ...
              className={cn(
                ...
                n.read ? "bg-card border-border opacity-70" : "bg-card border-primary/20 hover:border-primary/40",
              )}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="secondary" className={cn("text-[10px] rounded-full border-0", cfg.color)}>{cfg.label}</Badge>
